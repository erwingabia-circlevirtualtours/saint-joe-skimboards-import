    ...
    <section id="size-finder" className="py-20 bg-slate-50 relative border-b border-slate-200">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          ...
          <div className="lg:col-span-6 bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 space-y-6 shadow-sm">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center bg-slate-100 rounded-lg p-1 border border-slate-200 text-xs font-mono">
                  <button
                    ...
                    className={`px-2.5 py-0.5 rounded transition-colors ${unit === 'lbs' ? 'bg-slate-900 text-white font-bold' : 'text-slate-600 hover:text-slate-900'}`}
                  >
                    LBS
                  </button>
                  <button
                    ...
                    className={`px-2.5 py-0.5 rounded transition-colors ${unit === 'kg' ? 'bg-slate-900 text-white font-bold' : 'text-slate-600 hover:text-slate-900'}`}
                  >
                    KG
                  </button>
                </div>
              ...
              <div className="flex items-center justify-between bg-slate-50 p-4 rounded-2xl border border-slate-200 mb-3">
                <span className="text-xs font-mono text-slate-500">Current Weight:</span>
                <span className="font-display font-black text-3xl text-slate-900">
                  {weight} <span className="text-sm font-mono text-slate-500">{unit.toUpperCase()}</span>
                </span>
              </div>
          ...
          <div className="lg:col-span-6 bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 flex flex-col justify-between shadow-sm relative overflow-hidden">
            ...
            <div className="space-y-6 relative z-10">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200">
                  <span className="text-[10px] font-mono text-slate-500 block uppercase font-bold">RECOMMENDED LENGTH</span>
                  <span className="font-display font-black text-xl text-slate-900 block mt-1">{recommendation.size}</span>
                </div>

                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200">
                  <span className="text-[10px] font-mono text-slate-500 block uppercase font-bold">FOIL THICKNESS</span>
                  <span className="font-display font-black text-xl text-sky-700 block mt-1">{recommendation.thickness}</span>
                </div>
              </div>
              ...
              <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-xl bg-white border border-slate-200 flex items-center justify-center font-bold text-xs text-sky-700 p-1 overflow-hidden">
            ...
            <div className="pt-6 border-t border-slate-200 flex items-center justify-between text-xs font-mono text-slate-600 mt-6">
              <span>Want this shape in a custom resin tint?</span>
              <a href="#custom-builder" className="text-sky-700 font-bold hover:underline flex items-center gap-1">
                Customize in Studio →
              </a>
            </div>
